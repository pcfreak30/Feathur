<div id="Status">
    ...
    <div align="center">This page automatically updates every {%?RefreshTime} seconds. Network uptime may not be 100%
        accurate.
    </div>
    <br><br>
                ...
                <tr>
                    <td>
                        <div align="center"><img
                                    src="./templates/status/{%if isempty|server[status] == true}offline{%/if}{%if isempty|server[status] == false}online{%/if}.png"
                                    style="width:25px;height:25px;"></div>
                    </td>
                    <td>
                        {%if isempty|server[display_hs] == false}<a
                                href="history.php?id={%?server[id]}">{%?server[name]}</a>{%/if}
                    ...
                    <td>
                        ...
                        {%if isempty|server[display_network_uptime] == false}{%if isempty|server[display_hardware_uptime] == false}
                        <br>{%/if}{%/if}
                    ...
                    <td>
                        <div id="box-data">
                            <div style="width:{%?server[percent]}%;float:left;">
                                ...
                                <div align="center"><strong>Load:</strong> {%if isset|server[load_average] == false}
                                    N/A{%/if}{%?server[load_average]}</div>
                                {%/if}
                                {%if isempty|server[display_bandwidth] == false}
                                <div align="center"><strong>BW:</strong> {%if isset|server[bandwidth] == false}
                                    N/A{%/if}{%?server[bandwidth]} MB/s
                                </div>
                                {%/if}
                            </div>
                            ...
                            <div style="width:{%?server[percent]}%;float:left;margin-left:10px;">
                                ...
                                <div class="progress" style="padding:0;margin:0;">
                                    <div class="bar bar-warning"
                                         style="width: {%?server[percent_used_memory]}%;padding-top:5px;">U
                                    </div>
                                    <div class="bar bar-success"
                                         style="width: {%?server[percent_free_memory]}%;padding-top:5px;">F
                                    </div>
                                </div>
                            </div>
                            {%/if}
                            ...
                            <div style="width:{%?server[percent]}%;float:right;margin-left:10px;">
                                ...
                                <div class="progress" style="padding:0;margin:0;">
                                    <div class="bar bar-warning"
                                         style="width: {%?server[percent_used_disk]}%;padding-top:5px;">U
                                    </div>
                                    <div class="bar bar-success"
                                         style="width: {%?server[percent_free_disk]}%;padding-top:5px;">F
                                    </div>
                                </div>
                            </div>
                            {%/if}
