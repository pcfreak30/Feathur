...
abstract class CPHPDatabaseRecord extends CPHPBase
{
    public $fill_query = "";
    public $verify_query = "";
    public $table_name = "";
    ...
    public function RetrieveChildren($type, $field)
    {
        ...
        if ($parent_type !== "CPHPDatabaseRecord") {
            $parent_type = ($parent_type === false) ? "NONE" : $parent_type;
            $classname = get_class($this);
            throw new TypeException("{$classname}.RetrieveChildren expected 'type' argument of parent-type CPHPDatabaseRecord, but got {$parent_type} instead.");
        }
